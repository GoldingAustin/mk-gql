...
export function mergeConfigs(args, config) {
  ...
  return {
    ...
    roots: args["--roots"] ? args["--roots"].split(",").map((s) => s.trim()) : config.roots,
    excludes: args["--excludes"] ? args["--excludes"].split(",").map((s) => s.trim()) : config.excludes,
    modelsOnly: !!args["--modelsOnly"] || config.modelsOnly,
    forceAll: !!args["--force"] || config.force,
    namingConvention: args["--dontRenameModels"] ? "asis" : config.namingConvention,
    header: args["--header"] || headerConfigValues, // if multiple headers are passed in config, chain them up to pass on to apollo cli
    fieldOverrides: args["--fieldOverrides"] ? parseFieldOverrides(args["--fieldOverrides"]) : config.fieldOverrides
  }
}
...
const parseFieldOverrides = (fieldOverrides) => {
  return fieldOverrides
    ...
    .map((item) => {
      ...
      if (override.length !== 3) throw new Error("--fieldOverrides used with invalid override: " + item)
